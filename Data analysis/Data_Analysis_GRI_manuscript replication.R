...
bd <- paste(getwd(), 'output/', sep = '/')

packages <- c("car", "ggplot2", "mgcv", "texreg", "plyr", "plm", "flexmix",
              "Hmisc", "censReg", "ineq", "lmtest", "stringr", "nnet", "arm",
              "effects", "psych", "rms", "multcomp", "xtable", "pglm",
              "multiwayvcov", "Rmisc", "MASS")
...
# Merging datasets
# Corrections to data. In the code used in the Control sessions there was an
# error: the ideo1-10 scores were not registered correctly in the dataset. This
# did not affect any aspect of the experimental trial (as the data is only
# recorded for control purposes). For this reason, the data for the ideo1-10
# scores are erased from the data frame. If necesarry to use the data for
# statistical analysis, the correct data can be surched for by looking into how
# the original files were created)
...
for (i in 1:nrow(mydf)) {
    ...
    mydf$max_other_prop[i] <-
        if (!is.na(mydf$prop1[i]))
            apply(mydf[i, var.ideo], 1, max, na.rm = TRUE)
    else
        NA
    mydf$min_other_prop[i] <-
        if (!is.na(mydf$prop1[i]))
            apply(mydf[i, var.ideo], 1, min, na.rm = TRUE)
    else
        NA
}
rm(others, id)
...
for(i in 1:nrow(mydf)) {
    ...
    mydf$zero.prop[i] <-
        ifelse(is.na(mydf$prop1[i]), NA, ifelse(0 %in% prop.v, 1 , 0))
}
...
mydf[, "generosity"] <-
    ifelse(mydf$group_id == 1,
           mydf$pd1 - 3,
           ifelse(mydf$group_id == 2, mydf$pd2 - 3 , mydf$pd3 - 3))
...
tmp <- by(mydf, interaction(mydf$ugroup, mydf$period),  function(x) {
    for (i in 1:nrow(x))
    {
        ...
        x$prop.ideo.diff[i] <-
            ifelse(is.na(x$prop_id[i]), NA, abs(x$sp[i] - x$sp[x$group_id == x$prop_id[i]]))
        x$prop.same.gender[i] <-
            ifelse(is.na(x$prop_id[i]),
                   NA,
                   ifelse(x$gender[i] == x$gender[x$group_id == x$prop_id[i]], 1, 0))
        x$prop.same.race[i] <-
            ifelse(is.na(x$prop_id[i]),
                   NA,
                   ifelse(x$race2[i] == x$race2[x$group_id == x$prop_id[i]], 1, 0))
        x$prop.same.race.all[i] <-
            ifelse(is.na(x$prop_id[i]), NA, ifelse(x$race[i] == x$race[x$group_id == x$prop_id[i]], 1, 0))

        #Offers made to each participant every round
        prop_p <- numeric(0)
        for (j in 1:3)
            prop_p[j] <- which(x$group_id == j)
        ...
        for (w in 1:3)
            x$prop_dummy[w] <-
            ifelse(is.na(x$prop_id[w]),
                   NA,
                   ifelse(x$group_id[w] == x$prop_id[w], 1, 0))
...
mydf <-
    merge(
        mydf,
        mydf[mydf$userperiod == maxp, c("uid", "pd1", "pd2", "pd3", "generosity")],
        by.x = "uid",
        by.y = "uid",
        suffixes = c("", "all")
    )
...
mydf.l <-
    reshape(
        mydf[, c(
            "treatment", "id",  "userperiod", "round", "uid", "umg",
            "mg_id", "mg", "group", "ugroup", "group_id", "gender",
            "race", "race2", "g.gender", "g.race", "g.sp.sd" , "sp",
            "generosityall", "vote", "p.offered", "zero.prop",
            vars
        )],
        idvar = c(
            "treatment", "id", "userperiod", "round", "mg", "uid" ,
            "umg", "mg_id", "group", "ugroup", "group_id", "gender",
            "race", "race2", "g.gender", "g.race", "g.sp.sd", "sp",
            "generosityall", "vote", "p.offered", "zero.prop"
        ),
        timevar = "recipient",
        varying = list(
            prop = paste0("prop", 1:3),
            go1 = paste0("go", 1:3),
            ro = paste0("ro", 1:3),
            spo = paste0("spo", 1:3)
        ),
        direction = "long",
        new.row.names = 1:(nrow(mydf) * 3)
    )

for (i in 1:nrow(mydf.l)) {
    mydf.l$me[i] <- if (mydf.l$group_id[i] == mydf.l$recipient[i])
        1
    else
        0
}
...
#View(check)
tmp <-
    by(mydf.l, interaction(mydf.l$ugroup, mydf.l$userperiod, mydf.l$group_id),  function(x) {
        for (i in 1:nrow(x))
        {
...
mydf.l <-
    mydf.l[order(mydf.l$userperiod, mydf.l$umg,  mydf.l$ugroup, mydf.l$group_id), ]
rm(tmp)

#Difference in Differences, distance with the receiver and other.
mydf.l$did.sp <- NA
...
base.r1.df.o <-
    subset(mydf, treatment == "Control" & round == 1 & prop_dummy == 0)
...
props <- hist(treat.r1.dfl.o$prop1, freq = F, col = "blue", xaxt = "n", main = "", xlab = "pounds") + axis(side = 1, at = props$mids, labels = c(0:12))
#ggsave("hist_props_others.png")
...
pr <-
    pr + xlab("Proposer's Race") + ylab("pounds") + ggtitle("Offers")
pr <-
    pr + scale_fill_discrete(name = "Same Race", labels = c("Different", "Same"))
...
qplot(
    ...
    geom = c("point", "smooth"),
    method = "lm",
    shape = factor(g.gender),
    colour = factor(g.gender),
    ylab = "pounds",
    xlab = "Absolute value ideological distance" ,
    main = "Offers to other participants ",
    na.rm = TRUE
) + facet_grid(g.gender ~ .)
...
#Table male subjects
type1.1 <-
    treat.r1.dfl.o$gender == "male" &
    treat.r1.dfl.o$go == "female" & treat.r1.dfl.o$g.gender == 1
type2.1 <-
    treat.r1.dfl.o$gender == "male" &
    treat.r1.dfl.o$go == "male" & treat.r1.dfl.o$g.gender == 1
type3.1 <-
    treat.r1.dfl.o$gender == "male" &
    treat.r1.dfl.o$go == "female" & treat.r1.dfl.o$g.gender == 2
type4.1 <-
    treat.r1.dfl.o$gender == "male" &
    treat.r1.dfl.o$go == "male" & treat.r1.dfl.o$g.gender == 2


#Table Gender Interactions
type1.2 <-
    treat.r1.dfl.o$gender == "female" &
    treat.r1.dfl.o$go == "female" & treat.r1.dfl.o$g.gender == 1
type2.2 <-
    treat.r1.dfl.o$gender == "female" &
    treat.r1.dfl.o$go == "male" & treat.r1.dfl.o$g.gender == 1
...
require(ggplot2)
p <- ggplot(treat.r1.df, aes(max_other_prop, min_other_prop)) +
    scale_fill_gradient(low ="grey90", high = "black")
p <- p + stat_bin2d(bins = 20) + xlab("Larger offer") +
    ylab("Smaller offer") + xlim(c(0, 15)) + ylim(c(0, 15)) + theme_bw()
p <- p + geom_text(aes(3.97, 0.5, label = "SPNE"), colour = "blue") +
    theme(text = element_text(size = 24))
p
ggsave(
    filename = paste0(bd, "offers.png", sep = ""),
    width = 9,
    height = 6
)
...
lm.M1 <- ols(prop1~ideo.diff + same.g + same.r,
             data = treat.r1.dfl.o, y = T, x = T)
lm.M1.cl <- robcov(lm.M1, treat.r1.dfl.o$umg)
lm.M1.cl


#DV: Offer  - Definition 1 with controles
lm.M2 <- ols(prop1 ~ ideo.diff + ideo.diff.o + same.g + same.g.o + same.r+
                 same.g.o + same.r.o, data = treat.r1.dfl.o, y = T, x = T)
lm.M2.cl <- robcov(lm.M2, treat.r1.dfl.o$umg)
lm.M2.cl


lm.M3 <- ols(prop1 ~ ideo.diff + same.g + same.r + ideo.diff.o + same.g.o +
                 same.r.o + sp + gender + race2 , data = treat.r1.dfl.o,
             y = T, x = T)
...
log.p <- lrm(non.zero ~ ideo.diff +  same.g + same.r + ideo.diff.o + same.g.o +
                 same.r.o + sp + gender + race2 , data = treat.r1.dfl.o,
             x = T, y = T )
...
log.v1 <- lrm(vote ~ prop.ideo.diff + prop.same.gender + prop.same.race + sp +
                  gender + race2, data = treat.r1.df.o, x = T, y = T)
log.v1.cl <- robcov(log.v1, treat.r1.df.o$umg)
log.v1.cl


log.v2 <- lrm(vote ~ prop.ideo.diff + prop.same.gender + prop.same.race + sp +
                  gender + race2 + p.offered, data = treat.r1.df.o, x = T, y = T)
...
for (i in 1:30) {
    lm <-
        lm(
            ...
            data = subset(treat.r1.dfl.o, treat.r1.dfl.o$mg_id == i)
        )
    summary(lm)
    ideo.mg[i] <- lm$coef[2]
...
plot(ideo.mg,
     ylab = "Coef. Diff SP P-R",
     xlab = "Matching Group",
     cex.lab = 1.5) + abline(h = 0, col = "red")# +  theme(text=element_text(size=24))
dev.off()
...
for (i in 1:30) {
    lm <-
        lm(
            prop1 ~ ideo.diff.o + ideo.diff + same.g + same.r,
            data = subset(treat.r1.dfl.o, treat.r1.dfl.o$mg_id == i)
        )
    summary(lm)
    names(lm)
...
for (i in 1:30) {
    log.M1 <-
        glm(
            non.zero ~ ideo.diff + same.g + same.r ,
...
plot(ideo.partner, ylab = "Coef. Diff SP P-R", xlab = "Matching Group") + abline(h =
                                                                                     0, col = "red")
...
for (i in 1:nrow(mydf)) {
    ...
    if (mydf$treatment[i] == "GRI") {
        mydf$ideo.dist.A[i] <-
            abs(mydf$sp[i] - mydf[[paste0("spo", others[1], sep = "")]][[i]])
        mydf$ideo.dist.B[i] <-
            abs(mydf$sp[i] - mydf[[paste0("spo", others[2], sep = "")]][[i]])
        var.ideo <- c("ideo.dist.A", "ideo.dist.B")
        mydf$closer.ideo[i] <-
            ifelse(mydf$ideo.dist.A[i] == mydf$ideo.dist.B[i] ,
                   "Equal" ,
                   names(which.min(mydf[i, var.ideo])))

        props.i.want <- paste0("prop.", c("A", "B"), sep = "")
        mydf$offer.more[i] <-
            ifelse(mydf$prop.A[i] == mydf$prop.B[i] ,
                   "Equal" ,
                   names(which.max(mydf[i, props.i.want])))
    }
    else {
...
mydf$closer.ideo[mydf$closer.ideo == "ideo.dist.A"] <-
    "A" #simplification of values
...
tmp <- by(mydf, interaction(mydf$ugroup, mydf$period),  function(x) {
    ...
    for (i in 1:nrow(x)) {
        ...
        x$v.favour[i] <-
            ifelse(is.na(x$prop1[i]), NA, ifelse(x$vote[others[1]] == "1", "A" , "B"))
...
coalition.t <-
    table(coalition.df.me$coalition.vote[coalition.df.me$treatment == "GRI"])
...
plot.df <- ddply(treat.r1.dfl.o, .(ideo.diff), summarise, prop = mean(prop1))

p <- ggplot(plot.df, aes(ideo.diff, prop)) + theme_bw()
p <- p + geom_point() +
    labs(x = "Absolute value ideological distance", y = "Mean offer") +
    theme(text=element_text(size = 24))
p <- p + geom_hline(yintercept = mean(treat.r1.dfl.o$prop1), linetype = "dashed",
                    color = "red") +
    geom_hline(yintercept=mean(base.dfl.o$prop1, na.rm=T), color = "blue")
p

ggsave(filename = paste0(bd, "mean_offers_ideo_treat.png", sep = ""),
       width = 6, height = 6)
...
sp <- qplot(ideo.diff, prop1, data = base.dfl.o, geom = c("point", "smooth"),
          ...
          xlab = "Absolute value ideological distance",
          main = "Offers to other participants - Baseline ",
          na.rm = TRUE)
sp <- sp + geom_hline(yintercept = mean(treat.r1.dfl.o$prop1),
                      linetype = "dashed", color = "red")

sp <- sp + geom_hline(yintercept = mean(base.dfl.o$prop1, na.rm = T),
                      color = "darkgreen") + theme_bw()

sp
ggsave(filename = paste0(bd, "offers_ideo_base.png", sep = ""),
       width = 6, height = 6)
...
t.gr <-
    table(mydf$gender[mydf$userperiod == 0 & mydf$treatment == "GRI"],
          mydf$race[mydf$userperiod == 0 & mydf$treatment == "GRI"]) # A will be rows, B will be columns
...
lm.M1.ps <- ols(prop1 ~ ideo.diff * prosocial + ideo.diff.o * prosocial +
                    same.g * prosocial + same.g.o * prosocial + same.r * prosocial +
                    same.g.o * prosocial + same.r.o * prosocial,
                data = treat.r1.dfl.o, y = T, x = T)
...
lm.M2.ps <- ols(prop1 ~ ideo.diff * prosocial + ideo.diff.o * prosocial +
                    same.g * prosocial + same.g.o * prosocial + same.r * prosocial +
                    same.g.o * prosocial + same.r.o * prosocial + sp +
                    gender + race2 , data = treat.r1.dfl.o, y = T, x = T)
...
p.log.v1 <- lrm(vote ~ prop.ideo.diff + prop.same.gender + prop.same.race +
                   sp + gender + race2, data = base.r1.df.o, x = T, y = T)
...
p.log.v2 <- lrm(vote ~ prop.ideo.diff + prop.same.gender + prop.same.race +
                    sp + gender + race2 + p.offered,
                data = base.r1.df.o, x = T, y = T)
...
#partner
cl1 <- clogit(non.zero ~ ideo.diff +  same.g + same.r + ideo.diff.o +
                  same.g.o + same.r.o + strata(uid),
              data = treat.r1.dfl.o)
summary(cl1)
#vote
cl2 <- clogit(vote ~ prop.ideo.diff + prop.same.gender + prop.same.race +
                  strata(uid), data = treat.r1.df.o)
summary(cl2)


cl3 <- clogit(vote ~ prop.ideo.diff + prop.same.gender + prop.same.race +
                  p.offered + strata(uid), data = treat.r1.df.o)
summary(cl3)
...
#DV: Offer  - M3 basic covariates + 3er player controles + proposer characteristics
t1.lm.M3 <- ols(prop1 ~ ideo.diff + same.g + same.r + ideo.diff.o + same.g.o +
                    same.r.o + sp + gender + race2 ,
                data = treat.r1.dfl.o.05, y = T, x = T)
t1.lm.M3.cl <- robcov(t1.lm.M3, treat.r1.dfl.o.05$umg)

t2.lm.M3 <- ols(prop1 ~ ideo.diff + same.g + same.r + ideo.diff.o + same.g.o +
                    same.r.o + sp + gender + race2 ,
                data = treat.r1.dfl.o.610, y = T, x = T)
...
t1.log.p <- lrm(non.zero ~ ideo.diff +  same.g + same.r + ideo.diff.o +
                    same.g.o + same.r.o + sp + gender + race2,
                data = treat.r1.dfl.o.05, x = T, y = T)
t1.log.p.cl <- robcov(t1.log.p, treat.r1.dfl.o.05$umg)

t2.log.p <- lrm(non.zero ~ ideo.diff +  same.g + same.r + ideo.diff.o +
                    same.g.o + same.r.o + sp + gender + race2,
                data = treat.r1.dfl.o.610, x = T, y = T)
t2.log.p.cl <- robcov(t2.log.p, treat.r1.dfl.o.610$umg)


#DV: Vote  - M6  Model on acceptance of offers accounting for amount offered
t1.log.v2 <- lrm(vote ~ prop.ideo.diff + prop.same.gender + prop.same.race +
                    sp + gender + race2 + p.offered,
                data = treat.r1.df.o.05, x = T, y = T)
t1.log.v2.cl <- robcov(t1.log.v2, treat.r1.df.o.05$umg)

t2.log.v2 <- lrm(vote ~  prop.ideo.diff + prop.same.gender + prop.same.race +
                     sp + gender + race2 + p.offered,
                 data = treat.r1.df.o.610, x = T, y = T)
...
#### Interaction models
#DV: Offer  - M3 basic covariates + 3er player controles + proposer characteristics
int.lm.M3 <- ols(prop1 ~ ideo.diff + same.g * g.gender + same.r * g.race +
                     ideo.diff.o + same.g.o + same.r.o + sp + gender + race2,
                 data = treat.r1.dfl.o, y = T, x = T)
...
##### Partner and vote models
# DV: M4 Non-Zero offer ==1 Partner Selection
int.log.p <- lrm(non.zero ~ ideo.diff +  same.g * g.gender + same.r * g.race +
                     ideo.diff.o + same.g.o + same.r.o + sp + gender + race2,
                 data = treat.r1.dfl.o, x = T, y = T)
int.log.p.cl <- robcov(int.log.p, treat.r1.dfl.o$umg)

#DV: Vote  - M6  Model on acceptance of offers accounting for amount offered
int.log.v2 <- lrm(vote ~  prop.ideo.diff + prop.same.gender * g.gender +
                     prop.same.race * g.race + sp + gender + race2 + p.offered,
                 data = treat.r1.df.o, x = T, y = T)
...
#### Only mixed gender groups
#DV: Offer  - M3 basic covariates + 3er player controles + proposer characteristics
g.lm.M3 <- ols(prop1 ~ ideo.diff + same.g + same.r + ideo.diff.o + same.g.o +
                   same.r.o + sp + gender + race2 ,
               data = treat.r1.dfl.o.g, y = T, x = T)
g.lm.M3.cl <- robcov(g.lm.M3, treat.r1.dfl.o.g$umg)

##### Partner and vote models
# DV: M4 Non-Zero offer ==1 Partner Selection
g.log.p <- lrm(non.zero ~ ideo.diff +  same.g + same.r + ideo.diff.o +
                   same.g.o + same.r.o + sp + gender + race2,
               data = treat.r1.dfl.o.g, x = T, y = T)
g.log.p.cl <- robcov(g.log.p, treat.r1.dfl.o.g$umg)

#DV: Vote  - M6  Model on acceptance of offers accounting for amount offered
g.log.v2 <- lrm(vote ~  prop.ideo.diff + prop.same.gender + prop.same.race +
                    sp + gender + race2 + p.offered,
                data = treat.r1.df.o.g, x = T, y = T)
...
#DV: Offer  - M3 basic covariates + 3er player controles + proposer characteristics
r.lm.M3 <- ols(prop1 ~ ideo.diff + same.g + same.r + ideo.diff.o + same.g.o +
                   same.r.o + sp + gender + race2 ,
               data = treat.r1.dfl.o.r, y = T, x = T)
r.lm.M3.cl <- robcov(r.lm.M3, treat.r1.dfl.o.r$umg)


##### Partner and vote models
# DV: M4 Non-Zero offer ==1 Partner Selection
r.log.p <- lrm(non.zero ~ ideo.diff +  same.g + same.r + ideo.diff.o +
                   same.g.o + same.r.o + sp + gender + race2 ,
               data = treat.r1.dfl.o.r, x = T, y = T)
r.log.p.cl <- robcov(r.log.p, treat.r1.dfl.o.r$umg)


#DV: Vote  - M6  Model on acceptance of offers accounting for amount offered
r.log.v2 <- lrm(vote ~  prop.ideo.diff + prop.same.gender + prop.same.race +
                    sp + gender + race2 + p.offered,
                data = treat.r1.df.o.r, x = T, y = T)
...
c.data <- treat.r1.dfl.o[, c("prop1", "non.zero",  "ideo.diff", "ideo.diff.o",
                            "same.g", "same.r", "same.g.o", "same.r.o", "sp",
                         "gender", "race2", "umg")]
...
results <- boot_mg_lm(10000, eq=prop1~ideo.diff + ideo.diff.o + same.g +
                          same.g.o + same.r + same.r.o + sp + gender + race2)
...
hist(coef.ideo.diff,
     freq = F,
     xlab = "Bootstrapped Coef. for Ideo. Dist.  P-R",
     main = "")
...
ll.o <- mean - 1.96 * sd
ul.o <- mean + 1.96 * sd
hist(coef.ideo.diff.o,
     freq = F,
     xlab = "Bootstrapped Coef. for Ideo. Dist. P-3rd",
     main = "")
...
results.glm <- boot_mg_glm(10000, eq = non.zero ~ ideo.diff + ideo.diff.o + same.g +
                             same.g.o + same.r + same.r.o + sp + gender + race2)
...
ll <- mean - 1.96 * sd
ul <- mean + 1.96 * sd
hist(coef.glm.gender,
     freq = F,
     xlab = "Bootstrapped Coef. for Same Gender",
     main = "")
abline(v = ll, col = "red", lwd = 1)
abline(v = ul, col = "red", lwd = 1)
dev.off()
...
##############################################
lm.M1.race <- ols(prop1 ~ ideo.diff + same.g + same.r.all,
                  data = treat.r1.dfl.o, y = T, x = T)
lm.M1.race.cl <- robcov(lm.M1.race, treat.r1.dfl.o$umg)
lm.M1.race.cl

lm.M2.race <- ols(prop1 ~ ideo.diff + ideo.diff.o + same.g + same.g.o +
                      same.r.all + same.g.o + same.r.o.all,
                  data = treat.r1.dfl.o, y = T, x = T)
lm.M2.race.cl <- robcov(lm.M2.race, treat.r1.dfl.o$umg)
lm.M2.race.cl

lm.M3.race <- ols(prop1 ~ ideo.diff + same.g + same.r.all + ideo.diff.o + same.g.o +
                      same.r.o.all + sp + gender + race,
                  data = treat.r1.dfl.o, y = T, x = T)
lm.M3.race.cl <- robcov(lm.M3.race, treat.r1.dfl.o$umg)
lm.M3.race.cl

log.p.race <- lrm(non.zero ~ ideo.diff +  same.g + same.r.all + ideo.diff.o +
                      same.g.o + same.r.o.all + sp + gender + race,
                  data = treat.r1.dfl.o, x = T, y = T)
log.p.race.cl <- robcov(log.p.race, treat.r1.dfl.o$umg)
log.p.race.cl


log.v1.race <- lrm(vote ~ prop.ideo.diff + prop.same.gender +
                       prop.same.race.all  + sp + gender + race,
                   data = treat.r1.df.o, x = T, y = T)
log.v1.race.cl <- robcov(log.v1.race, treat.r1.df.o$umg)
log.v1.race.cl


log.v2.race <- lrm(vote ~  prop.ideo.diff + prop.same.gender +
                       prop.same.race.all + sp + gender + race + p.offered,
                   data = treat.r1.df.o, x = T, y = T)
...
poisson1 <- glm(prop1.p ~ ideo.diff +  same.g + same.r + ideo.diff.o + same.g.o +
                    same.r.o + sp + gender + race2,
                data = treat.r1.dfl.o, family = poisson)
summary(poisson1)


negbin1 <- glm.nb(prop1.p ~ ideo.diff +  same.g + same.r + ideo.diff.o +
                      same.g.o + same.r.o + sp + gender + race2,
                  data = treat.r1.dfl.o)
...
##########################################
#DV: Offer  - Definition 1 same group
lm.M1.sp <- ols(prop1 ~ ideo.diff + same.g * g.sp.sd + same.r,
                data = treat.r1.dfl.o, y = T, x = T)
...
#DV: Offer  - Definition 1 with controles
lm.M2.sp <- ols(prop1 ~ ideo.diff + same.g * g.sp.sd + ideo.diff.o + same.g.o +
                    same.r + same.g.o + same.r.o,
                data = treat.r1.dfl.o, y = T, x = T)
lm.M2.sp.cl <- robcov(lm.M2.sp, treat.r1.dfl.o$umg)
lm.M2.sp.cl

lm.M3.sp <- ols(prop1 ~ ideo.diff + same.g * g.sp.sd + ideo.diff.o + same.g.o +
                    same.r + same.g.o + same.r.o + sp + gender + race2,
                data = treat.r1.dfl.o, y = T, x = T)
...
# Partner and vote models
log.p.sp <- lrm(non.zero ~ ideo.diff + same.g * g.sp.sd + ideo.diff.o +
...
log.p.sp.cl
 ...
 #DV: Vote  - Basic model with clustered se for correlation  within groups
log.v1.sp <- lrm(vote ~  prop.ideo.diff + prop.same.gender * g.sp.sd +
                     prop.same.race + sp + gender + race2,
                 data = treat.r1.df.o, x = T, y = T)
log.v1.sp.cl <- robcov(log.v1.sp, treat.r1.df.o$umg)
log.v1.sp.cl

log.v2.sp <- lrm(vote ~ prop.ideo.diff + prop.same.gender * g.sp.sd +
                     prop.same.race + sp + gender + race2 + p.offered,
                 data = treat.r1.df.o, x = T, y = T)
...
##########################################
#DV: Offer  - Definition 1 same group
lm.M1.spr <- ols(prop1 ~ ideo.diff + same.g + same.r * g.sp.sd,
                 data = treat.r1.dfl.o, y = T, x = T)
...
#DV: Offer - Definition 1 with controles
lm.M2.spr <- ols(prop1 ~ ideo.diff + same.g + ideo.diff.o + same.g.o +
                     same.r * g.sp.sd + same.g.o + same.r.o,
                 data = treat.r1.dfl.o, y = T, x = T)
lm.M2.spr.cl <- robcov(lm.M2.spr, treat.r1.dfl.o$umg)
lm.M2.spr.cl

lm.M3.spr <- ols(prop1 ~ ideo.diff + same.g + ideo.diff.o + same.g.o +
...
log.p.spr <- lrm(non.zero ~ ideo.diff + same.g + ideo.diff.o + same.g.o +
                     same.r * g.sp.sd + same.g.o + same.r.o + sp + gender + race2,
                 data = treat.r1.dfl.o, x = T, y = T)
...
#DV: Vote  - Basic model with clustered se for correlation  within groups
log.v1.spr <- lrm(vote ~  prop.ideo.diff + prop.same.gender +
                      prop.same.race * g.sp.sd  + sp + gender + race2,
                  data = treat.r1.df.o, x = T, y = T)
log.v1.spr.cl <- robcov(log.v1.spr, treat.r1.df.o$umg)
log.v1.spr.cl


log.v2.spr <- lrm(vote ~  prop.ideo.diff + prop.same.gender +
                      prop.same.race * g.sp.sd  + sp + gender + race2 + p.offered,
                  data = treat.r1.df.o, x = T, y = T)
...
lm.FM2 <- ols(prop1 ~ ideo.diff + same.g + same.r + ideo.diff.o + same.g.o +
                  same.r.o, data = treat.dfl.o, y = T, x = T)
lm.FM2.cl <- robcov(lm.FM2, treat.dfl.o$umg)
lm.FM2.cl

lm.FM3 <- ols(prop1 ~ ideo.diff + same.g + same.r + ideo.diff.o + same.g.o +
                  same.r.o + sp + gender + race2 ,
              data = treat.dfl.o, y = T, x = T)
lm.FM3.cl <- robcov(lm.FM3, treat.dfl.o$umg)
lm.FM3.cl

log.FMp <- lrm(non.zero ~ ideo.diff +  same.g + same.r + ideo.diff.o + same.g.o +
                   same.r.o + sp + gender + race2,
               data = treat.dfl.o, x = T, y = T)
log.FMp.cl <- robcov(log.FMp, treat.dfl.o$umg)
log.FMp.cl

log.FMv1 <- lrm(vote ~ prop.ideo.diff + prop.same.gender + prop.same.race +
                    sp + gender + race2, data = treat.df.o, x = T, y = T)
log.FMv1.cl <- robcov(log.FMv1, treat.df.o$umg)
log.FMv1.cl


log.FMv2 <- lrm(vote ~  prop.ideo.diff + prop.same.gender + prop.same.race +
                    sp + gender + race2 + p.offered,
...
log.p <- lrm(non.zero ~ ideo.diff + ideo.diff.o + same.g + same.g.o +
                 same.r + same.r.o + gender + race2 + sp,
             data = treat.dfl.o, x = T, y = T)
...
#DV: Vote  - Basic model with clustered se for correlation  within groups
log.v1 <- lrm(vote ~ prop.ideo.diff + prop.same.gender + prop.same.race +
                  gender + race2 + sp, data = treat.df.o, x = T, y = T)
log.v1.cl <- robcov(log.v1, treat.df.o$umg)
log.v1.cl


log.v2 <- lrm(vote ~  prop.ideo.diff + prop.same.gender + prop.same.race +
                  gender + race2 + sp + p.offered,
              data = treat.df.o, x = T, y = T)
log.v2.cl <- robcov(log.v2, treat.df.o$umg)
log.v2.cl

log.v3 <- lrm(vote ~ p.offered +  prop.ideo.diff + prop.same.gender +
                  prop.same.race + gender + race2 + sp + sd.p.offered + prop.sp,
              data = treat.df.o, x = T, y = T)
log.v3.cl <- robcov(log.v3, treat.df.o$umg)
log.v3.cl

log.v4 <- lrm(vote ~  p.offered +  prop.ideo.diff + prop.same.gender +
                  prop.same.race + gender + race2 + sp + sd.p.offered *
                  p.offered + prop.sp, data = treat.df.o, x = T, y = T)
log.v4.cl <- robcov(log.v4, treat.df.o$umg)
log.v4.cl

log.v5 <- lrm(vote ~ p.offered +  prop.ideo.diff + prop.same.gender +
                  prop.same.race + gender + race2 + sp * sd.p.offered,
              data = treat.df.o, x = T, y = T)
